...
int taperedEval_old(struct position *pos) {
	...
	pos->BBblackpieces = pos->colours[BLACK];
	pos->BBwhitepieces = pos->colours[WHITE];
	pos->BBpawns = pos->pieces[PAWN];
	pos->BBknights = pos->pieces[KNIGHT];
	pos->BBbishops = pos->pieces[BISHOP];
	pos->BBrooks = pos->pieces[ROOK];
	pos->BBqueens = pos->pieces[QUEEN];
	pos->BBkings = pos->pieces[KING];
